...
#         time_str = time*10**9
        ...
        for filename in messages:
              ...
              try:
                  ...
                  if int(decoded_message['id']) in [1,2,3]:
                      ...
                      if (float(decoded_message['y']) != 91.0) and (float(decoded_message['x']) != 181.0):
    ...
    df5 = df4.withColumn('day', lit(int(day)))
    return df5
...
def main():
    ...
    if df:
        # df.orderBy("day").write.partitionBy("day").mode("append").parquet(f"exploration") 
        # df.write.csv('exploration.csv') 
        df.write.format('com.databricks.spark.csv').save('exploration.csv')
