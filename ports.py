...
def ports(spark, cell_width, cell_length):
    ...
    dfa = spark.read.format("csv").option("header", "true").option("delimiter", ",").option("inferschema", "true").load("anchor.csv*")
...
def main():
    ...
    dfp = ports(spark, cell_width, cell_length)
    
#     cell_map.write.mode("overwrite").parquet(f"/mnt/group05/utils/cell_map")
    # dfn.write.mode("overwrite").parquet(f"/mnt/group05/utils/neighbours")  
    dfp.write.mode("overwrite").parquet(f"ports")
